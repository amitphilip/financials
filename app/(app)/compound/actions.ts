...
async function collection() {
  ...
  return client.db("financials").collection("compound");
}

export async function saveCompound(data: SavedCompound) {
...
export async function loadCompound(): Promise<SavedCompound | null> {
  const ids = await resolveEffectiveUserId();
  if (!ids) throw new Error("Unauthorized");
  ...
  return JSON.parse(decrypt(doc.payload as string, ids.effectiveUserId)) as SavedCompound;
}
